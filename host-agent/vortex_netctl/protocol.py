...
class NameParams(Params): name:str=Field(pattern=r"^[A-Z0-9_-]{1,32}$")
...
class DeviceConfigParams(Params): device_name:str=Field(pattern=r"^[A-Z0-9_-]{1,32}$"); include_client_config:Literal[True]
...
def parse_request(line:bytes)->tuple[RpcRequest,Params]:
    ...
    if request.method in {"add_device","enable_device","disable_device","delete_device","rotate_device_uuid"}: cls=NameParams
    elif request.method in {"add_force_vpn","remove_force_vpn","add_force_direct","remove_force_direct"}: cls=DomainParams
    elif request.method=="test_destination": cls=DestinationParams
    elif request.method=="restore_backup": cls=BackupParams
    elif request.method=="get_devices" and request.params: cls=DeviceConfigParams

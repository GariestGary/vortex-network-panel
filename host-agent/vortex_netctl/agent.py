...
async def make_handler(controller):
    async def handle(reader, writer):
        ...
        try:
            ...
            elif method == "get_vpn_provider": result = controller.get_vpn_provider()
            elif method == "set_vpn_provider": result = controller.set_vpn_provider(params.provider)
...
async def main():
    ...
    try: controller.migrate_legacy_hyvpn(); controller.reconcile_vpn_provider()
    except Exception: logging.getLogger("vortex_netctl.agent").exception("VPN provider reconciliation failed")
